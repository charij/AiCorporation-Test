...
namespace aiCorporation.NewImproved
{
    ...
    public class SalesAgentFileRecordList
    {
        ...
        {
            var salSalesAgentList = new ConcurrentBag<SalesAgentBuilder>();
            Parallel.ForEach(m_lsafrSalesAgentFileRecordList.GroupBy(i => i.SalesAgentEmailAddress).ToArray(), new ParallelOptions{ MaxDegreeOfParallelism = 16 }, salesManGroup =>
            {
                SalesAgentBuilder saCurrentSalesAgent = new SalesAgentBuilder
                {
                    SalesAgentEmailAddress = salesManGroup.First().SalesAgentEmailAddress,
                    SalesAgentName = salesManGroup.First().SalesAgentName
                };
                
                foreach (var clientGroup in salesManGroup.GroupBy(i => i.ClientIdentifier))
                {
                    ClientBuilder cClient = new ClientBuilder
                    {
                        ClientIdentifier = clientGroup.First().ClientIdentifier,
                        ClientName = clientGroup.First().ClientName
                    };
                
                    foreach (var record in clientGroup)
                    {
                        cClient.BankAccountList.Add(new BankAccountBuilder
                        {
            ...
            return new SalesAgentList(salSalesAgentList.Select(i => i.ToSalesAgent()).ToList());
        }
